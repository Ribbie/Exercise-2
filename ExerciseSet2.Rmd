...
*1. Looking at the table showing the Eigenvalue and the % of explained variance we can see that the first 6 components cumulatively explain 52.29% of total variance.

*2. Looking at the table showing the weights of factor rotations of variables vs. components, we can see that the following 4 variables have a weight higher than 0.8: 5 yr T-Notes US (0.93), 10yr T-Notes (0.93), Treasury Bonds (0.86), 2 yr T-note US (0.85)

*3. I would name the components as follows:
  +Component 1: Bonds
  +Component 2: Currencies
  +Component 3: European stock indices
  +Component 4: North American Stock Indices
  +Component 5: Foreign exchange rates
  +Component 6: Oil
  +Component 7: Metals
  +Component 8: Oilseeds
  +Component 9: Precious metals
  +Component 10: Japan/Hong Kong stock indices

*4. In the crisis years there are less principal components, meaning that a smaller number of components are in order to explain futures pricing. In effect, the correlations between the different variables increase considerably. In the pre-crisis years there was less correlation between variables, hence the number of principal components was higher.

In result, one would have to change its model to account for the differences in dynamics before the crisis vs. during the crisis 
